...
class TwitterScraperSnscrape:
    ...
    def save_to_csv(self, df, filename='twitter_data_snscrape_2025.csv'):
        ...
        if not df.empty:
            ...
            df_copy['hashtags'] = df_copy['hashtags'].apply(lambda x: ','.join(x) if isinstance(x, list) else '')
    ...
    try:
        ...
        if not df.empty:
            ...
            for idx, (i, tweet) in enumerate(top_tweets.iterrows(), 1):
                ...
                print(f"   Likes: {tweet['like_count']}, Retweets: {tweet['retweet_count']}, Replies: {tweet['reply_count']}")
